...
export function DraftingTool({ kind }: { kind: DraftKind }) {
  ...
  return <SafeAreaView style={styles.page}><AppHeader active="tools"/><ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
    ...
    <View style={styles.card}>
      ...
      <Pressable accessibilityRole="button" disabled={!valid || loading} onPress={() => void generate()} style={[styles.primary, (!valid || loading) && styles.disabled]}>{loading ? <ActivityIndicator color="#FFF"/> : <Text style={styles.primaryText}>{draft ? 'Дахин боловсруулах' : 'Ноорог бэлтгэх'}</Text>}</Pressable>
