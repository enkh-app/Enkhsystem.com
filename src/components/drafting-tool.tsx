...
export function DraftingTool({ kind }: { kind: DraftKind }) {
  const { t } = useI18n();
  const textOperations = useMemo(() => [['polish',t('draft.opPolish')],['shorten',t('draft.opShorten')],['expand',t('draft.opExpand')],['summarize',t('draft.opSummarize')],['translate',t('draft.opTranslate')],['formal',t('draft.opFormal')]] as const,[t]);
  const tones = useMemo(() => [['professional',t('draft.toneProfessional')],['friendly',t('draft.toneFriendly')],['formal',t('draft.toneFormal')],['concise',t('draft.toneConcise')]] as const,[t]);
  const documentTypes = useMemo(() => [['letter',t('draft.typeLetter')],['proposal',t('draft.typeProposal')],['report',t('draft.typeReport')],['memo',t('draft.typeMemo')],['other',t('draft.typeOther')]] as const,[t]);
  ...
  const persist = (result: DraftResult) => {
    ...
    else setNotice(t('draft.workspaceFailed'));
  };
  ...
  const saveEdit = () => {
    ...
    if (kind === 'document' && !model) { setNotice(t('draft.invalidDocument')); return; }
    ...
    if (saveWorkspace(next)) { backgroundWorkspaceSync.schedule(next); setNotice(t('draft.saved')); }
  };

  const exportDocx = async () => {
    const model = documentAfterEdit(draft, documentModel, { title, documentType });
    if (!model || exporting) { if (!model) setNotice(t('draft.checkBeforeExport')); return; }
    ...
    try {
      ...
      setNotice(`${filename}: ${t('draft.fileReady')}`);
    } catch { setError(t('draft.docxError')); }
  ...
  const copy = async () => {
    ...
    } catch { setNotice(t('draft.copyFailed')); }
  };
  ...
  return <SafeAreaView style={styles.page}><SeoHead title={seo[0]} description={seo[1]} path={seo[2]} /><AppHeader active="tools"/><ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
    ...
    <View style={styles.card}>
      {kind === 'text' && <><Text style={styles.label}>{t('draft.operation')}</Text><Chips items={textOperations} value={operation} onChange={setOperation}/><Field label={t('draft.textInput')} value={text} onChange={setText} large/>{operation === 'translate' && <Field label={t('draft.targetLanguage')} value={targetLanguage} onChange={setTargetLanguage}/>}<Field label={t('draft.instructions')} value={instructions} onChange={setInstructions}/></>}
      {kind === 'message' && <><Text style={styles.label}>{t('draft.tone')}</Text><Chips items={tones} value={tone} onChange={setTone}/><Field label={t('draft.recipient')} value={recipient} onChange={setRecipient}/><Field label={t('draft.intent')} value={intent} onChange={setIntent} large/><Field label={t('draft.context')} value={context} onChange={setContext} large/></>}
      {kind === 'document' && <><Text style={styles.label}>{t('draft.type')}</Text><Chips items={documentTypes} value={documentType} onChange={setDocumentType}/><Field label={t('draft.title')} value={title} onChange={setTitle}/><Field label={t('draft.purpose')} value={purpose} onChange={setPurpose} large/><Field label={t('draft.documentContext')} value={context} onChange={setContext} large/></>}
      <Pressable accessibilityRole="button" disabled={!valid || loading} onPress={() => void generate()} style={[styles.primary, (!valid || loading) && styles.disabled]}>{loading ? <><ActivityIndicator color="#FFF"/><Text style={styles.loadingText}>{t('draft.generating')}</Text></> : <Text style={styles.primaryText}>{draft ? t('common.regenerate') : t('common.generate')}</Text>}</Pressable>
      {!!error && <View accessibilityLiveRegion="polite" style={styles.error}><Text style={styles.errorText}>{error}</Text><Pressable accessibilityRole="button" onPress={() => void generate()} style={styles.retry}><Text style={styles.retryText}>{t('common.retry')}</Text></Pressable></View>}
    </View>
    {!!draft && <View style={styles.result}><View style={styles.resultHeader}><View><Text style={styles.label}>{t('draft.ready')}</Text>{kind === 'message' && <Text style={styles.safeNote}>{t('draft.notSent')}</Text>}{kind === 'document' && <Text style={styles.safeNote}>{t('draft.docxNote')}</Text>}</View><View style={styles.resultActions}><Pressable accessibilityRole="button" onPress={() => void copy()} style={styles.copy}><Text style={styles.copyText}>{t('common.copy')}</Text></Pressable>{kind === 'document' && <Pressable accessibilityRole="button" disabled={exporting} onPress={() => void exportDocx()} style={[styles.exportButton, exporting && styles.disabled]}><Text style={styles.exportText}>{exporting ? t('draft.generating') : t('draft.docx')}</Text></Pressable>}</View></View><TextInput accessibilityLabel={t('draft.editLabel')} multiline value={draft} onChangeText={setDraft} onBlur={saveEdit} style={styles.draft}/>{!!notice && <Text accessibilityLiveRegion="polite" style={styles.notice}>{notice}</Text>}</View>}
...
function Field({ label, value, onChange, large=false }: { label:string; value:string; onChange(value:string):void; large?:boolean }) {
  const { t } = useI18n();
  return <View style={styles.field}><Text style={styles.label}>{label}</Text><TextInput accessibilityLabel={label} multiline={large} value={value} onChangeText={onChange} style={[styles.input,large&&styles.large]} placeholder={t('draft.placeholder')} placeholderTextColor="#829AB1"/></View>;
}
