...
test('cloud workspace uses explicit import and revision-protected sync', () => {
  ...
  assert.match(api, /api\/workspace\/import/); assert.match(api, /expectedRevision/); assert.match(screen, /Local history import/); assert.match(screen, /Cloud руу sync/); assert.match(screen, /replaceWorkspaceSafely/);
});
