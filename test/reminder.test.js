...
test('tools advertise the real reminder without claiming email or push',()=>{const tools=read('src/app/actions.tsx');assert.match(tools,/route:\s*'\/action-reminder'/);assert.doesNotMatch(tools,/const upcoming = \['Сануулга'\]/);assert.doesNotMatch(read('src/app/action-reminder.tsx'),/deliveryChannel:'email'|deliveryChannel:'push'/);});
